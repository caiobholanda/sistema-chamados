...
function renderChamadoItem(c) {
  ...
  return `
    <div class="chamado-item prioridade-${c.prioridade || 'sem'}${encerrado ? ' chamado-encerrado' : ''}" data-id="${c.id}" tabindex="0" role="button" aria-label="Abrir chamado #${c.id}">
      ...
      <div class="chamado-desc"><span style="font-size:.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--text-secondary);display:block;margin-bottom:.25rem">Descrição do problema</span>${c.descricao}</div>
  ...
  document.getElementById('modal-body').innerHTML = `
    <div style="display:grid;gap:1rem">
      ...
      <div>
        ...
        <div class="modal-desc"><span style="font-size:.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--text-secondary);display:block;margin-bottom:.35rem">Descrição do problema</span>${c.descricao}</div>
