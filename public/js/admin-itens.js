          ...
          ${itens.map(item => `
            <tr>
              ...
              <td style="white-space:nowrap">
                <button class="btn btn-acao-editar btn-sm" onclick="abrirModalEditar(${item.id})">Editar</button>
                <button class="btn btn-acao-excluir btn-sm" onclick="confirmarDeletar(${item.id}, ${JSON.stringify(item.nome).replace(/"/g,'&quot;')})">Excluir</button>
              </td>
            </tr>
          `).join('')}
        </tbody>
        ...
        <tbody>
          ${lista.map(item => `
            <tr>
              ...
              <td style="white-space:nowrap">
                <button class="btn btn-acao-editar btn-sm" onclick="abrirModalMicros(${item.id})">Editar</button>
                ${isMaster ? `<button class="btn btn-acao-excluir btn-sm" onclick="confirmarDeletarMicro(${item.id}, ${JSON.stringify(item.setor + ' - ' + item.usuario).replace(/"/g,'&quot;')})">Excluir</button>` : ''}
              </td>
            </tr>
          `).join('')}
        </tbody>
            ...
            ${itens.map(item => `
              <tr${autoUrgente(item) ? ' style="background:rgba(220,38,38,.04)"' : ''}>
                ...
                <td style="white-space:nowrap">
                  ...
                  ${isMaster ? `<button class="btn btn-acao-excluir btn-sm" onclick="confirmarDeletarToner(${item.id},${JSON.stringify(item.nome).replace(/"/g,'&quot;')})">Excluir</button>` : ''}
                </td>
              </tr>
            `).join('')}
          </tbody>
        ...
        <tbody>
          ${lista.map(item => `
            <tr>
              ...
              <td style="color:var(--text-secondary)">${esc(item.localizacao) || '—'}</td>
              <td style="white-space:nowrap">
                <button class="btn btn-acao-editar btn-sm" onclick="abrirModalImpressora(${item.id})">Editar</button>
                ${isMaster ? `<button class="btn btn-acao-excluir btn-sm" onclick="confirmarDeletarImpressora(${item.id},${JSON.stringify(item.nome).replace(/"/g,'&quot;')})">Excluir</button>` : ''}
              </td>
            </tr>
          `).join('')}
        </tbody>
...
function renderEquipamentos(lista, reservaLegado = []) {
  ...
  // Agrupar EQ-XXXX por nome
  const gruposEq = {};
  for (const eq of lista) {
    if (!gruposEq[eq.nome]) gruposEq[eq.nome] = { categoria: eq.categoria, itens: [] };
    gruposEq[eq.nome].itens.push(eq);
  }

  // Montar linhas unificadas: primeiro EQ, depois reserva que não tem EQ com mesmo nome
  const nomesEq = new Set(Object.keys(gruposEq));
  const reservaSemEq = reservaLegado.filter(i => !nomesEq.has(i.nome));

  const totalItens = Object.keys(gruposEq).length + reservaSemEq.length;
  const badge = document.getElementById('badge-reserva');
  if (badge) badge.textContent = (lista.length + reservaSemEq.length) || '';
  ...
  const linhasEq = Object.entries(gruposEq).map(([nome, g]) => {
    const contagem = {};
  ...
  const linhasReserva = reservaSemEq.map(item => {
    const qtd = item.qtd_geral ?? 0;
    const qtdHtml = qtd === 0
      ? `<span style="color:var(--danger);font-weight:600">0</span>`
      : `<span style="color:var(--success);font-weight:600">${qtd}</span>`;
    return `
      <tr>
        <td style="font-weight:500">${esc(item.nome)}</td>
        <td style="color:var(--text-secondary);font-size:.82rem">${esc(item.especificacao) || '—'}</td>
        <td style="text-align:center">${qtdHtml}</td>
        <td><span style="font-size:.72rem;color:var(--text-muted);font-style:italic">sem ID individual</span></td>
        <td style="white-space:nowrap">
          <button class="btn btn-acao-editar btn-sm" onclick="abrirModalReservaLegado(${item.id})">Editar</button>
          <button class="btn btn-primary btn-sm" onclick="abrirMovimentacao(${item.id})">Movimentação</button>
          <button class="btn btn-secondary btn-sm" onclick="abrirHistoricoMovimentacoes(${item.id},${JSON.stringify(item.nome).replace(/"/g,'&quot;')})">Histórico</button>
          ${isMaster ? `<button class="btn btn-acao-excluir btn-sm" onclick="confirmarDeletarReservaLegado(${item.id},${JSON.stringify(item.nome).replace(/"/g,'&quot;')})">Excluir</button>` : ''}
        </td>
      </tr>
    `;
  }).join('');
  ...
  el.innerHTML = `
    ...
    ${lista.length > 0 ? '<div style="font-size:.78rem;color:var(--text-muted);margin-top:.5rem;padding:0 .25rem">Clique no nome ou em "Ver IDs" para ver as unidades individuais.</div>' : ''}
  `;
}
          ...
          return `
            <div style="display:flex;align-items:center;gap:1rem;padding:.85rem 1rem;border:1px solid var(--border);border-radius:0;background:var(--surface-1);transition:box-shadow .15s" onmouseover="this.style.boxShadow='0 2px 8px rgba(0,0,0,.08)'" onmouseout="this.style.boxShadow=''">
              ...
              <div style="display:flex;gap:.4rem;flex-shrink:0">
                ...
                <button class="btn btn-acao-editar btn-sm" onclick="fecharEqHist();eqEditar(${eq.id})">Editar</button>
                ${isMaster ? `<button class="btn btn-acao-excluir btn-sm" onclick="fecharEqHist();eqDeletar(${eq.id},${JSON.stringify(eq.codigo).replace(/"/g,'&quot;')})">✕</button>` : ''}
              </div>
            </div>
          `;
        }).join('')}
    ...
    <form id="form-micros" style="display:flex;flex-direction:column;gap:.8rem;padding:1.25rem 1.5rem">
      <div class="form-row-2">
        <div class="form-group">
          ...
          <input class="form-control" id="fm-setor" type="text" list="dl-setores" value="${esc(unesc(item.setor || ''))}" placeholder="Ex: CONTROLADORIA">
        </div>
        <div class="form-group">
          <label class="form-label">Usuário <span style="color:var(--danger)">*</span></label>
          <input class="form-control" id="fm-usuario" type="text" list="dl-usuarios" value="${esc(unesc(item.usuario || ''))}" placeholder="Nome do usuário">
        </div>
      </div>
      <div class="form-row-2">
        <div class="form-group">
          ...
          <div style="display:flex;gap:.4rem;align-items:stretch">
            <input class="form-control" id="fm-tipo-equipamento" type="text" list="dl-cfg-equipamento" value="${esc(unesc(item.tipo_equipamento || ''))}" placeholder="Ex: NOTEBOOK, DESKTOP" style="flex:1">
            ${!isEdit ? `<button type="button" class="cfgti-quick-add" onclick="_quickAddConfTI('equipamento','Tipo de Equipamento','fm-tipo-equipamento',this)" title="Adicionar novo valor">+</button>` : ''}
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">Nobreak/Nº</label>
          <div style="display:flex;gap:.4rem;align-items:stretch">
            <input class="form-control" id="fm-nobreak" type="text" list="dl-cfg-nobreak" value="${esc(unesc(item.nobreak || ''))}" placeholder="Ex: SMS 700VA / Nº série" style="flex:1">
            ${!isEdit ? `<button type="button" class="cfgti-quick-add" onclick="_quickAddConfTI('nobreak','Nobreak/Nº','fm-nobreak',this)" title="Adicionar novo valor">+</button>` : ''}
          </div>
        </div>
      </div>
      <div class="form-row-2">
        <div class="form-group">
          ...
          <div style="display:flex;gap:.4rem;align-items:stretch">
            <input class="form-control" id="fm-processador" type="text" list="dl-cfg-processador" value="${esc(unesc(item.processador || ''))}" placeholder="Ex: I5 12500" style="flex:1">
            ${!isEdit ? `<button type="button" class="cfgti-quick-add" onclick="_quickAddConfTI('processador','Processador','fm-processador',this)" title="Adicionar novo valor">+</button>` : ''}
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">Memória</label>
          <div style="display:flex;gap:.4rem;align-items:stretch">
            <input class="form-control" id="fm-memoria" type="text" list="dl-cfg-memoria" value="${esc(unesc(item.memoria || ''))}" placeholder="Ex: 8GB" style="flex:1">
            ${!isEdit ? `<button type="button" class="cfgti-quick-add" onclick="_quickAddConfTI('memoria','Memória','fm-memoria',this)" title="Adicionar novo valor">+</button>` : ''}
          </div>
        </div>
      </div>
      <div class="form-row-2">
        <div class="form-group">
          ...
          <div style="display:flex;gap:.4rem;align-items:stretch">
            <input class="form-control" id="fm-so" type="text" list="dl-cfg-so" value="${esc(unesc(item.sistema_operacional || ''))}" placeholder="Ex: WIN 11" style="flex:1">
            ${!isEdit ? `<button type="button" class="cfgti-quick-add" onclick="_quickAddConfTI('so','S.O.','fm-so',this)" title="Adicionar novo valor">+</button>` : ''}
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">HD/SSD</label>
          <div style="display:flex;gap:.4rem;align-items:stretch">
            <input class="form-control" id="fm-hd" type="text" list="dl-cfg-hd_ssd" value="${esc(unesc(item.hd_ssd || ''))}" placeholder="Ex: SSD 256" style="flex:1">
            ${!isEdit ? `<button type="button" class="cfgti-quick-add" onclick="_quickAddConfTI('hd_ssd','HD/SSD','fm-hd',this)" title="Adicionar novo valor">+</button>` : ''}
          </div>
        </div>
      </div>
      <div class="form-row-2">
        <div class="form-group">
          ...
          <div style="display:flex;gap:.4rem;align-items:stretch">
            <input class="form-control" id="fm-office" type="text" list="dl-cfg-office" value="${esc(unesc(item.office || ''))}" placeholder="Ex: H & B 2021" style="flex:1">
            ${!isEdit ? `<button type="button" class="cfgti-quick-add" onclick="_quickAddConfTI('office','Office','fm-office',this)" title="Adicionar novo valor">+</button>` : ''}
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">TAG</label>
          <input class="form-control" id="fm-tag" type="text" value="${esc(unesc(item.tag || ''))}" placeholder="Código da tag">
        </div>
      </div>
      <div class="form-row-2">
        <div class="form-group">
          ...
          <div style="display:flex;gap:.4rem;align-items:stretch">
            <input class="form-control" id="fm-entradas" type="text" list="dl-cfg-entradas_monitor" value="${esc(unesc(item.entradas_monitor || ''))}" placeholder="Ex: VGA-D.P." style="flex:1">
            ${!isEdit ? `<button type="button" class="cfgti-quick-add" onclick="_quickAddConfTI('entradas_monitor','Entradas Monitor','fm-entradas',this)" title="Adicionar novo valor">+</button>` : ''}
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">Modelo Monitor</label>
          <div style="display:flex;gap:.4rem;align-items:stretch">
            <input class="form-control" id="fm-monitor" type="text" list="dl-cfg-modelo_monitor" value="${esc(unesc(item.modelo_monitor || ''))}" placeholder="Ex: DELL" style="flex:1">
            ${!isEdit ? `<button type="button" class="cfgti-quick-add" onclick="_quickAddConfTI('modelo_monitor','Modelo Monitor','fm-monitor',this)" title="Adicionar novo valor">+</button>` : ''}
          </div>
        </div>
      </div>
      <div class="form-row-2">
        <div class="form-group">
          <label class="form-label">Status</label>
          <input class="form-control" id="fm-status" type="text" value="${esc(unesc(item.status || ''))}" placeholder="Ex: NOVO, CONCLUIDO">
        </div>
        <div class="form-group">
      ...
      <div class="form-row-2">
        <div class="form-group">
          ...
          <input class="form-control" id="fm-troca" type="text" value="${esc(unesc(item.data_troca || ''))}" placeholder="Ex: 2024">
        </div>
        <div class="form-group">
          <label class="form-label">Atualização Win11</label>
          <input class="form-control" id="fm-win11" type="text" value="${esc(unesc(item.atualizacao_win11 || ''))}" placeholder="Ex: CONCLUIDO">
        </div>
      </div>
      <div class="form-group">
        <label class="form-label">Observação</label>
        <input class="form-control" id="fm-obs" type="text" value="${esc(unesc(item.observacao || ''))}" placeholder="Observações">
      </div>
